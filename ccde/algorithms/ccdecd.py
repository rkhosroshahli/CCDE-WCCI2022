...
class DEIterator:
    ...
    def __iter__(self):
        ...
        while self.fes < de.maxfes and self.iteration_ccde < de.maxiters - ((2 * de.dims * 10)/de.popsize):
            ...
            for cidx in range(de.NC):
                cluster = self.population[cidx * self.CS:
                                          (cidx+1) * self.CS]
                centroid = np.sum(cluster, axis=0) / self.CS
                ...
                self.population[de.popsize - de.NC + cidx,
                                :] = np.copy(centroid)
        ...
        self.best_fitness = self.fitness[self.best_idx]
        print(f"The last best error of CCDE: {self.best_fitness:e}")
        varmax, varmin = np.amin(self.population[:10], axis=0), np.amax(
            self.population[:10], axis=0)

        center_position = np.zeros(de.dims)
        best_pos_fitness, = de.evaluate([center_position])
        self.fes += 1
        if best_pos_fitness < self.best_fitness:
            best_position = center_position
            self.best_fitness = best_pos_fitness

        cd_better_counter = 0

        while(self.fes < de.maxfes and self.iteration_cd < (2 * de.dims * 10)/de.popsize):
            for d in range(de.dims):
                ld = varmax[d] - varmin[d]
                c1, c2 = best_position, best_position
                c1[d] = varmin[d] + ld/4
                c1_fitness, = de.evaluate([c1])
                self.fes += 1
                if self.fes == de.maxfes:
                    if c1_fitness < self.best_fitness:
                        cd_better_counter += 1
                        best_position = c1
                        self.best_fitness = c1_fitness
                    break
                c2[d] = varmax[d] - ld/4
                c2_fitness, = de.evaluate([c2])
                self.fes += 1
                self.best_fitness = min(
                    self.best_fitness, c1_fitness, c2_fitness)
                if c1_fitness == self.best_fitness:
                    cd_better_counter += 1
                    best_position = c1
                    varmax[d] -= ld/2
                elif c2_fitness == self.best_fitness:
                    cd_better_counter += 1
                    best_position = c2
                    varmin[d] += ld/2
                else:
                    # print(self.best_fitness, c1_fitness, c2_fitness)
                    varmin[d] += ld/4
                    varmax[d] -= ld/4

                if self.best_fitness < self.fitness[self.best_idx]:
                    self.population[self.best_idx] = np.copy(best_position)
                    self.fitness[self.best_idx] = self.best_fitness
                if self.fes % de.popsize == 1:
                    self.all_mean_fs.append(np.mean(self.fitness))
                    self.all_best_fs.append(self.best_fitness)
                yield self
        ...
        self.all_mean_fs.append(np.mean(self.fitness))
        self.all_best_fs.append(self.best_fitness)
        print(f"The last best error of CCDE+CD: {self.best_fitness:e}")
        print("Times CD found better position:",
              cd_better_counter, "out of", de.maxfes - ccde_fes)
...
class CCDECD:
    ...
    def __init__(self, fobj, bounds, strategy='rand1bin', mutation=0.5, crossover=0.9, maxfes=3e+06, popsize=100, seed=None, NC=None, fstar=0):
    ...
    def evaluate(self, P):
        return [self.fobj(ind)-self.fstar for ind in P]
    ...
    def solve(self, show_progress=False):
        ...
        for step in iterator:
            ...
            all_mean_fs = step.all_mean_fs
            all_best_fs = step.all_best_fs
        return P[idx].reshape(1, -1), bestfitness, all_best_fs, all_mean_fs


class PCCDECD(CCDECD):
